...
- If API is changed - make sure OpenAPI specs regenerated by running `mvn verify`
- When running the application locally use profiles "local" and "init-db"
- For any significant behavior/API/security change, add or update automated tests in the same PR (unit/service/controller as appropriate).
...
- No secrets in the repo. Use application-local.properties.
